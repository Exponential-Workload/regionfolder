...
export class CustomRegion {
  ...
  constructor(
    startRegionTag: RegionTag,
    endRegionTag: RegionTag = RegionTag.Unknown(),
  ) {
...
export class RegionProvider {
  ...
  public getRegions(document: vscode.TextDocument): {
    completedRegions: CustomRegion[];
    errors: string[];
  } {
    const languageId = document.languageId;
    console.log(
      `RegionProvider.getRegions - Getting regions for language ${languageId}.`,
    );
    ...
    console.log(
      `RegionProvider.getRegions - ${completedRegions.length} completed regions.`,
    );
    return {
